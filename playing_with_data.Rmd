...
for(j in 1:ncol(DayOfWeek)){
  ...
  for(i in 2:nrow(DayOfWeek)){
    week_index = ifelse(DayOfWeek[i-1,j]==7 & DayOfWeek[i,j]==1, week_index+1,  week_index) # identify the change of week
...
rm(Day.Index, MDayOfWeek, MWeek.Index, Week.Index, DayOfWeek, LA_matrix)
...
```{r}
DayLeq <- list()
# Take "CT16_01" for example 
df <- LA.Seq[["CT16_01"]] %>% filter(`Week-Day` == "1-6")
unique(LA.Seq[["CT16_01"]]$`Week-Day`)
exp_sum <- 0
Time_dur <- nrow(df) * 0.0625
ti <- 0.0625
for(i in 1:nrow(df)){
  exp_sum <- exp_sum + (1/Time_dur)*ti*10^(df[i, 2]/10)
}

10 * log(exp_sum, base = 10)
